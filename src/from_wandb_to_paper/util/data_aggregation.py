...
def aggregate_run_histories(
    ...
    run_histories: Dict[Dict, Union[str, pd.DataFrame]],
...
def _get_class_fraction_for_metrics_name(
    metrics_name: str, class_fractions: Dict[str, float]
) -> float:
...
def calculate_class_weighted_mean(
    metrics_table: pd.DataFrame, class_fractions: Dict[str, float]
) -> pd.Series:
    result = {}
    for (experiment_name, aggregate), metrics_dict in metrics_table.to_dict().items():
        ...
        for metrics_name, metrics_value in metrics_dict.items():
            value += (
                _get_class_fraction_for_metrics_name(
                    metrics_name, class_fractions=class_fractions
                )
                * metrics_value
            )
